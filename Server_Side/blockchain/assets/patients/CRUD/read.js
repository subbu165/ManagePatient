...
function getV5cIDs(req, res)
{
	...
	tracing.create('ENTER', 'GET blockchain/assets/patients', []);
	configFile = reload(__dirname+'/../../../../configurations/configuration.js');
	var ids = [];
	...
	var querySpec = {
					  ...
					  "params": {
					    "type": 1,
					    "chaincodeID": {
					      "name": configFile.config.patient_log_name
					    },
					    "ctorMsg": {
					      "function": "get_patient_logs",
					      "args": []
					    },
	...
	request(options, function(error, response, body)
	{
		...
		{
			var data = JSON.parse(body.result.message).patient_logs;
			...
			if(typeof data != undefined)
			{
				...
				{
					res.end();
					tracing.create('EXIT', 'GET blockchain/assets/patients', '');
				}
			}
			else
			{
				...
				tracing.create('ERROR', 'GET blockchain/assets/patients', 'Unable to get blockchain assets');
			}
		}
		else
		{
...
function getV5cDetails(ids, i, req, res)
{
	...
	request(options, function(error, response, body)
	{
		...
		if (!body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			{
				res.end();
				tracing.create('EXIT', 'GET blockchain/assets/patients', 'Got '+(i+1)+' patients');
			}
		}
		else
		{
			console.log("PATIENTS READ ERROR", body.error)
			...
			else
			{
				...
				res.end(JSON.stringify(error))
				tracing.create('ERROR', 'GET blockchain/assets/patients', 'Unable to get v5c '+ids[i]);
			}
		}
	})
}
